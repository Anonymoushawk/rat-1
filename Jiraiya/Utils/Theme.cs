...
class FirefoxSubTabControl : TabControl
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        for (int i = 0; i <= TabPages.Count - 1; i++)
        {
            ...
            {
                using (SolidBrush B = new SolidBrush(Helpers.GreyColor(250)))
                {
                    G.FillRectangle(B, new Rectangle(GetTabRect(i).Location.X - 2, GetTabRect(i).Location.Y - 2, GetTabRect(i).Width, GetTabRect(i).Height + 1));
                }

                using (Pen P = new Pen(Color.FromArgb(1, 130, 254), 3)) //nice blue
                {
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        for (int i = 0; i <= TabPages.Count - 1; i++)
        {
            ...
            if (SelectedIndex == i)
            {
                ...
                using (SolidBrush B = new SolidBrush(Color.FromArgb(1, 130, 254))) //nice blue
                {
                    G.FillRectangle(B, new Rectangle(TabRect.Location.X - 3, TabRect.Location.Y + 1, 3, TabRect.Height - 2));
                }
